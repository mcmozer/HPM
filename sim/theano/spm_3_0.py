...
def param_init_spm(options, params, prefix='spm'):
    ...
    #forget0 = numpy.log(numpy.exp(gamma0)-1.0) DEBUG
    forget0 = numpy.log(gamma0)


    # determine initial input rate (assuming only bias is nonzero)
    lambda0 = 1.0 / (gamma0 * options['arch_spm_init_input_timescale']) - 1.0
    ...
    print('DEBUG: using exp not softplus -- init gamma',gamma0, 'init lambda',lambda0)

    # biases are ordered: (1) input gate, (2) time const, (3) input weights
    b = numpy.concatenate([input0 * numpy.ones((n_hid)),         # input rate
                           forget0 * numpy.ones((n_hid)),        # forget rate
...
def spm_layer(tparams, state_below, xt, yt, options, prefix='spm', mask=None):
    ...
    def _step(m_, x_, delta_t_input, delta_t_output, o_, h_, g_, gamma_, lam_):
        # o_, h_, gamma_, lam_ have dim n_training_examples  X  n_spm
        # delta_t_input has dim n_training_examples

        hhat = tensor.exp(-delta_t_input[:,None]*gamma_) * h_
        ghat = tensor.exp(-delta_t_input[:,None]*gamma_*(lam_+1)) * g_
        squashed_hid = tensor.tanh(hhat-ghat)

        # x_ = W * x_t + b
        preact = x_ + tensor.dot(squashed_hid, tparams[_p(prefix, 'U')]) 

        z = tensor.tanh(_slice(preact,2,n_hid))
        h = hhat + z
        g = ghat + z

        #gamma = tensor.nnet.softplus(_slice(preact,1,n_hid)) DEBUG
        gamma = tensor.exp(_slice(preact,1,n_hid))
        hhat_pred = tensor.exp(-delta_t_output[:,None]*gamma) * h 

        #lam = tensor.nnet.softplus(_slice(preact,0,n_hid)) DEBUG
        lam = tensor.exp(_slice(preact,0,n_hid))
        ghat_pred = tensor.exp(-delta_t_output[:,None]*gamma) * g
        o = tensor.tanh(hhat_pred - ghat_pred)

        h = m_[:, None] * h + (1. - m_)[:, None] * h_
        g = m_[:, None] * g + (1. - m_)[:, None] * g_
        o = m_[:, None] * o + (1. - m_)[:, None] * o_

        return o, h, g, gamma, lam
    ...
    rval, updates = theano.scan(_step,
                        ...
                        outputs_info=[tensor.alloc(numpy_floatX(0.), # o_
                                          ...
                                          tensor.alloc(numpy_floatX(0.), # g_
                                                       n_samples,
                                                       n_hid),
                                          tensor.alloc(numpy_floatX(1.), # gamma_
                                                       n_samples,
                                                       n_hid),
                                          tensor.alloc(numpy_floatX(1.), # lambda_
                                                       n_samples,
                                                       n_hid)],
...
def train_model(
    ...
    arch_spm_init_input_timescale = 1.0, # intial input rate
