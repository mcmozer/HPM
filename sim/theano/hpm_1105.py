...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    alpha = 1.0 * numpy.ones((n_hid)) # DEBUG 7/23/16
    ...
    output_calibration = numpy.log(numpy.exp(1.0)-1.0)*numpy.ones((n_hid)) 
                                                                # anti soft-plus
    params[_p(prefix, 'output_calibration')] = (
                                     output_calibration.astype(config.floatX))
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    output_calibration = tensor.nnet.softplus(
                   tparams[_p(prefix,'output_calibration')]).dimshuffle(('x',0))
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        if (not options['arch_remap_input']):
            ...
            if (options['arch_hpm_recurrent']):
                ...
                marginal_intensity_input = (marginal_intensity_input / 
                               (output_calibration + marginal_intensity_input))
                net += tensor.dot(
	...
	# predict next event conditioned on timescale
        ...
        marginal_intensity_output = (marginal_intensity_output / 
                               (output_calibration + marginal_intensity_output))
...
def print_weights(tparams,options):
    ...
    if (options['arch_remap_input']):
        print('Wemb:\n',print_params['Wemb'])
    print('output_calibration:\n',
              numpy.log(1.0+numpy.exp(print_params['hpm_output_calibration'])))
