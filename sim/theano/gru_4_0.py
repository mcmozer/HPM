'''
3/26/2017 quickie implementation of Rob Lindsey's SURVIVAL PROCESS MEMORY (ctgru)
          as a variant of LSTM with exponential state-dependent forgetting rate
3/27/2017 version 2.1 with softplus and *gamma instead of /tau
3/28/2017 version 2.2
          initialized time scale U to 0.0 and bias to obtain longest time scale
          (via arch_spm_include_forgetting=TIME_SCALE)
4/5/2017  version 3.0 (forked from 2.2)
          replace input gate with input rate 
          2 state variables for memory (input and memory)
          arch_spm_init_{forget,input}_timescale
4/8/2017  version 4.0 (forked from 3.1)
          proper transfer of input to hidden activation
4/10/2017 version 4.1 - preserve forgotten activation and pass forward
4/12/2017 version 5.0 - GRU and contin time GRU
''' 
...
def param_init_ctgru(options, params, prefix='ctgru'):
    """
    Init the ctgru parameter:

    :see: init_params
    """
    n_hid = options['n_hid']
    W = numpy.concatenate([numpy.zeros((n_hid, n_hid)),
                           numpy.zeros((n_hid, n_hid)),
                           ortho_weight(n_hid)], axis=1)
    params[_p(prefix, 'W')] = W
    ...
    # determine initial forget rate (assuming only bias is nonzero) 
    gamma0 = 1.0 / options['arch_ctgru_init_forget_timescale']
    if (options['arch_ctgru_rate_function'] == 'softplus'):
        if (gamma0 > 100.):
            forget0 = gamma0
        else:
            forget0 = numpy.log(numpy.exp(gamma0)-1.0) 
    elif (options['arch_ctgru_rate_function'] == 'exp'):
        forget0 = numpy.log(gamma0)
    else:
        print('Error: invalid arch_ctgru_rate_function')
        quit()

    # determine initial absorption rate (assuming only bias is nonzero)
    lambda0 = 1.0 /  options['arch_ctgru_init_recurrent_timescale']
    if (options['arch_ctgru_rate_function'] == 'softplus'):
        recurrent0 = numpy.log(numpy.exp(lambda0)-1.0)
    else:
        recurrent0 = numpy.log(lambda0)

    # biases are ordered: 
    # (1) recurrent gate decay, (2) memory decay, (3) input function 
                          
    b = numpy.concatenate([recurrent0 * numpy.ones((n_hid)),     # recurrent decay
                           forget0 * numpy.ones((n_hid)),        # memory decay
...
###############################################################################
# ctgru (really a continuous-time GRU)
###############################################################################
...
def ctgru_layer(tparams, state_below, xt, yt, options, prefix='ctgru', mask=None):
    ...
    # this code is a bit redundant in order to allow the possibility that
    # the delta_t_output at step K is different than delta_t_input at step K+1

    def _step(m_, x_, delta_t_input, delta_t_output, h_, hhat_):
        # o_, h_, g_ have dim n_training_examples  X  n_ctgru
        # delta_t_input has dim n_training_examples

        # WARNING: ASSUMING THAT \Delta-t_out(k) = \Delta-t_in(k+1)
        # If this assumption isn't true, need to replace o_ with dynamic
        # computation
        # Because of this assumption, delta_t_input is not used

        # determine time constants moving forward
        # x_ = W * x_t + b
        ...
        if (options['arch_ctgru_rate_function'] == 'softplus'):
            rho =  tensor.nnet.softplus(_slice(x_, 0, n_hid) +
                     tensor.dot(h_aug, tparams[_p(prefix,'Ur')]))
            zeta = tensor.nnet.softplus(_slice(x_, 1, n_hid) +
                     tensor.dot(h_aug, tparams[_p(prefix,'Uz')]))
        else:
            rho =  tensor.exp(_slice(x_, 0, n_hid) +
                     tensor.dot(h_aug, tparams[_p(prefix,'Ur')]))
            zeta = tensor.exp(_slice(x_, 1, n_hid) +
                     tensor.dot(h_aug, tparams[_p(prefix,'Uz')]))

        if (options['arch_ctgru_include_delta_t']):
            hhat_ = tensor.concatenate([hhat_, delta_t_input[:,None], 
                                        delta_t_output[:,None]], axis=1)
        # compute event signal
        s = tensor.tanh(_slice(x_, 2, n_hid) +
                     tensor.dot(hhat_, tparams[_p(prefix,'Uh')]))

        # determine future (decayed) state
        zeta_decay = tensor.exp(-zeta * delta_t_output[:, None])
        rho_decay = tensor.exp(-rho * delta_t_output[:, None])
        h = zeta_decay * h_ + (1.0-zeta_decay) * s
        hhat = rho_decay * h_
    ...
    rval, updates = theano.scan(_step,
                        sequences=[mask, state_below, xt, yt],
                        outputs_info=[tensor.alloc(numpy_floatX(0.), # h_
                                                       n_samples,
                                                       n_hid),
                                          tensor.alloc(numpy_floatX(0.), # hhat_
                                                       n_samples,
                                                       n_hid)],
                        name=_p(prefix, '_layers'),
                        n_steps=nsteps)
...
def build_model(tparams, options):
    ...
    # this is the call to either gru_layer or hpm_layer
...
def train_model(
    ...
    arch_ctgru_rate_function = 'softplus', # softplus or exp 
    arch_ctgru_init_recurrent_timescale = 1.0, # intial absorption rate
    arch_ctgru_init_forget_timescale = 100.0, # initial forgetting rate
    arch_gru_include_delta_t = True, #include delta_t values in gru input 
    arch_ctgru_include_delta_t = False, # delta_t values included for CTGRU
    ...
    global layers
    if model_options['encoder'] == 'ctgru':
        layers = {'ctgru': (param_init_ctgru, ctgru_layer)}
    elif model_options['encoder'] == 'gru':
