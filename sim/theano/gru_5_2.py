...
def ctgru_layer(tparams, state_below, xt, yt, options, prefix='ctgru', mask=None):
    ...
    def _softmax_timeconst(tc,logprior):
    ...
    def _step(m, x_, delta_t_input, delta_t_output, h_, y_):
        ...
        r = _slice(x_, 2, n_hid) + tensor.dot(y_, tparams[_p(prefix,'Ur')])

        # credit assignment to timescales
        rho = _softmax_timeconst(r, tc_logprior)

        # retrieve memory
        retrieved_y_ = tensor.sum(h_ * rho, axis=1)

        # detect event signal
        q = tensor.tanh(_slice(x_, 0, n_hid) +
                     tensor.dot(retrieved_y_, tparams[_p(prefix,'Uq')]))

        # determine log(storage scale)
        if (options['arch_ctgru_ohat_for_sscale']):
            s = (_slice(x_, 1, n_hid) 
                           + tensor.dot(retrieved_y_, tparams[_p(prefix,'Us')]) )
        else:
            s = _slice(x_, 1, n_hid) + tensor.dot(y_, tparams[_p(prefix,'Us')])

        # credit assignment to timescales
        sigma = _softmax_timeconst(s, tc_logprior)
	...
	# decay hidden according to timescale
        ...
        # output is sum of activity across timescales

        y = tensor.sum(h,axis=1)
...
def train_model(
    ...
    arch_ctgru_ohat_for_sscale = False, # storage scale based on ohat (vs. o)?
