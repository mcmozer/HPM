...
def spm_layer(tparams, state_below, xt, yt, options, prefix='spm', mask=None):
    ...
    def _step(m_, x_, delta_t_input, delta_t_output, o_, h_, tau_):
        # o_, h_, and tau_ have dim n_training_examples  X  n_spm
        # delta_t_input has dim n_training_examples

        if (options['arch_spm_include_forgetting']):
            hhat = tensor.exp(-delta_t_input[:,None]/tau_) * h_
        else:
            hhat = h_
        ...
        if (options['arch_spm_include_forgetting']):
            tau = tensor.exp(_slice(preact,1,n_hid))
            o = tensor.tanh( tensor.exp(-delta_t_output[:,None]/tau) * h )
        else:
            o = tensor.tanh(h)
