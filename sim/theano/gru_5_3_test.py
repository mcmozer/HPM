...
4/20/2017 GRU_5_3: different computation of output gate
...
def init_params(options):
    ...
    if (options['arch_output_fn'] != '1-1'):
        hid_out = options['n_hid']
        if (options['encoder'] == 'ctgru' and
                                 options['arch_ctgru_include_nodecay_out']):
            hid_out *= 2
        params['U'] = (init_wt_mag / numpy.sqrt(options['n_out']) * 
                                  numpy.random.randn(hid_out,
...
def param_init_gru(options, params, prefix='gru'):
    ...
    Ur = ortho_weight(n_hid)
    Uz = ortho_weight(n_hid)
    Uh = ortho_weight(n_hid)
    # augment GRU input feature weights to include delta_t values
    if (options['arch_gru_include_delta_t']):
       Ur = numpy.concatenate([Ur, .01 * numpy.random.randn(2,n_hid)],axis=0)
       Uz = numpy.concatenate([Uz, .01 * numpy.random.randn(2,n_hid)],axis=0)
       Uh = numpy.concatenate([Uh, .01 * numpy.random.randn(2,n_hid)],axis=0)

    params[_p(prefix, 'Ur')] = Ur
    params[_p(prefix, 'Uz')] = Uz
    params[_p(prefix, 'Uh')] = Uh
    b = numpy.zeros((3 * n_hid,))
...
def gru_layer(tparams, state_below, xt, yt, options, prefix='gru', mask=None):
    ...
    def _step(m_, x_, delta_t_input, delta_t_output, h_):
        # h_ has dim n_training_examples  X  n_gru
        # delta_t_input has dim n_training_examples

        # include input and output delta_t values as GRU input features
        if (options['arch_gru_include_delta_t']):
        ...
        z = tensor.nnet.sigmoid(_slice(x_, 0, n_hid) +
                     tensor.dot(h_aug, tparams[_p(prefix,'Uz')]))
        r = tensor.nnet.sigmoid(_slice(x_, 1, n_hid) +
                     tensor.dot(h_aug, tparams[_p(prefix,'Ur')]))

        rh_ = r * h_
        if (options['arch_gru_include_delta_t']):
            rh_ = tensor.concatenate([rh_, delta_t_input[:,None], 
                                     delta_t_output[:,None]], axis=1)
        event = tensor.tanh(_slice(x_, 2, n_hid) +
                     tensor.dot(rh_, tparams[_p(prefix,'Uh')]))
        h = z * h_ + (1.0-z) * event
...
def param_init_ctgru(options, params, prefix='ctgru'):
    """
    ...
    Uq = ortho_weight(n_hid)
    Us = ortho_weight(n_hid)
    Ur = ortho_weight(n_hid)
    # augment GRU input feature weights to include nondecayed outputs
    if (options['arch_ctgru_include_nodecay_out']):
       Uq = numpy.concatenate([Uq, ortho_weight(n_hid)], axis=0)
       Us = numpy.concatenate([Us, ortho_weight(n_hid)], axis=0)
       Ur = numpy.concatenate([Ur, ortho_weight(n_hid)], axis=0)

    # augment GRU input feature weights to include delta_t values
    if (options['arch_ctgru_include_delta_t']):
       Uq = numpy.concatenate([Uq, .01 * numpy.random.randn(2,n_hid)],axis=0)
       Us = numpy.concatenate([Us, .01 * numpy.random.randn(2,n_hid)],axis=0)
       Ur = numpy.concatenate([Ur, .01 * numpy.random.randn(2,n_hid)],axis=0)

    params[_p(prefix, 'Uq')] = Uq
    params[_p(prefix, 'Us')] = Us
    params[_p(prefix, 'Ur')] = Ur

    # remapping of the embedding 

    # set initial bias for the decay over the full range of time scales
    decay_max = numpy.log(1.0/numpy.min(timescales))
    decay_min = numpy.log(1.0/numpy.max(timescales))
    #s0 = numpy.random.rand(n_hid,)*(decay_max-decay_min)+decay_min
    #r0 = numpy.random.rand(n_hid,)*(decay_max-decay_min)+decay_min
    s0 = (decay_max+decay_min)/2.0 
    r0 = s0
    b = numpy.concatenate([numpy.zeros(n_hid,),             # q - event signal
                           s0 * numpy.ones(n_hid,),         # s - storage scale
                           r0 * numpy.ones(n_hid,)], axis=0)# r - retrieval scale
    params[_p(prefix, 'b')] = b.astype(config.floatX)

    W = numpy.concatenate([numpy.identity(n_hid,dtype=config.floatX),
                           ortho_weight(n_hid),
                           ortho_weight(n_hid)], axis=1)
    params[_p(prefix, 'W')] = W

    if (options['arch_ctgru_include_priors']):
        params[_p(prefix,'tc_logprior')] = numpy.zeros(n_timescales)
    if (options['arch_ctgru_include_smoothc']):
        params[_p(prefix,'tc_smoothc')] = (numpy.ones(n_timescales) * 
                                            numpy.log(numpy.exp(1.0)-1.0))
...
def ctgru_layer(tparams, state_below, xt, yt, options, prefix='ctgru', mask=None):
    nsteps = state_below.shape[0]
    if state_below.ndim == 3:
    ...
    n_hid = options['n_hid']
    timescales = options['timescales']
    gamma = 1.0 / numpy_floatX(timescales)
    loggamma = numpy.log(gamma[None,:])
    n_timescales = len(timescales)

    if (options['arch_ctgru_include_priors']):
        tc_logprior = tparams[_p(prefix,'tc_logprior')]
    else:
        tc_logprior = numpy.zeros(n_timescales)
    tc_logprior = tc_logprior[None,:]
     
    if (options['arch_ctgru_include_smoothc']):
        tc_smoothc = tensor.nnet.softplus(tparams[_p(prefix,'tc_smoothc')])
    else:
        tc_smoothc = numpy.ones(n_timescales)
    tc_smoothc = tc_smoothc[None,:]
    ...
    def _softmax_timeconst(tc):
        sm = tensor.nnet.softmax(-(tc.reshape((-1, 1)) - loggamma)**2.0 
                                     * tc_smoothc + tc_logprior)
        return sm.reshape((-1,n_hid,n_timescales)).dimshuffle((0,2,1))
        # This code works too
        #loggamma2 = numpy.log(gamma[None,:,None])
        #dist = -(tc[:,None,:] - loggamma2)**2.0 + logprior[None,:,None]
        #dist = tensor.exp(dist - dist.max(axis=1)[:, None, :])
        #return dist / dist.sum(axis=1)[:, None, :]

    def _step(m, x_, delta_t_input, delta_t_output, h_, y_, y2_):
        # h_: n_examples X n_timescales X n_hid
        # y_: n_examples X n_hid

        aug_y_ = y_
        if (options['arch_ctgru_include_nodecay_out']):
            aug_y_ = tensor.concatenate([aug_y_, y2_], axis=1)
        if (options['arch_ctgru_include_delta_t']):
            aug_y_ = tensor.concatenate([aug_y_, delta_t_input[:,None], 
                                     delta_t_output[:,None]], axis=1)

        # detect event signal
        q = tensor.tanh(_slice(x_, 0, n_hid) +
                        tensor.dot(aug_y_, tparams[_p(prefix,'Uq')]))

        # determine log(storage scale)
        s = _slice(x_, 1, n_hid) + tensor.dot(aug_y_, tparams[_p(prefix,'Us')])

        # credit assignment to timescales
        sigma = _softmax_timeconst(s)

        # update memory 
	hhat = (1.0 - sigma) * h_ + sigma * q[:,None,:]

	# decay hidden according to timescale

        h = hhat * tensor.exp(-gamma[None,:,None] * delta_t_output[:,None,None])

        # determine log(retrieval scale)
        r = _slice(x_, 2, n_hid) + tensor.dot(aug_y_, tparams[_p(prefix,'Ur')])

        # credit assignment to timescales
        rho = _softmax_timeconst(r)

        # retrieve memory
        y = tensor.sum(h * rho, axis=1)
        y2 = tensor.sum(hhat * rho, axis=1)

	# clear out updates after end of sequence
        y = m[:, None] * y + (1. - m)[:, None] * y_
        y2 = m[:, None] * y2 + (1. - m)[:, None] * y2_
    ...
    rval, updates = theano.scan(_step,
							   ...
							   n_timescales,
                                                           n_hid),
                                              tensor.alloc(numpy_floatX(0.),  # y
                                                           n_examples,
                                                           n_hid),
                                              tensor.alloc(numpy_floatX(0.),  # y2
                                                           n_examples,
                                                           n_hid)],
                                name=_p(prefix, '_layers'),
                                n_steps=nsteps)
    if options['arch_ctgru_include_nodecay_out']:
        y = tensor.concatenate([rval[1], rval[2]], axis=2)
    else:
        y = rval[1]
    return y, rval[0] # return y, h
...
def build_model(tparams, options):
    ...
    # this is the call to either gru_layer or ctgru_layer
...
def train_model(
    ...
    arch_ctgru_include_priors = False, # allow priors over timescale to be learned
    arch_ctgru_include_smoothc = False, # allow smoothing constant to be learned
    arch_ctgru_include_delta_t = False, #include delta_t values in ct-gru input 
    arch_ctgru_include_nodecay_out = False, # feedback nondecayed outputs
    arch_gru_include_delta_t = True, #include delta_t values in gru input 
    n_hid=40,  # number of recurrent hidden units; ignored if arch_input_map_constraint!='none'
    ...
    encoder='ctgru',  # gru or ctgru
    saveto='weights/ctgru_model.npz',  # The best model will be saved there
    ...
    global layers
    if model_options['encoder'] == 'gru':
        layers = {'gru': (param_init_gru, gru_layer)}
    elif model_options['encoder'] == 'ctgru':
        layers = {'ctgru': (param_init_ctgru, ctgru_layer)}
    else:
