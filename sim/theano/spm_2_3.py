...
3/28/2017 version 2.3
          incorporate gamma_i, log(gamma_i) into input representation for unit i
'''
...
def param_init_spm(options, params, prefix='spm'):
    ...
    if (options['arch_spm_input_includes_gamma']):
        params[_p(prefix, 'V')] = numpy.zeros((3*n_hid))
        params[_p(prefix, 'Vlog')] = numpy.zeros((3*n_hid))

    # determine initial forgetting gamma value
    if (options['arch_spm_include_forgetting'] in {True, False}):
        forget0 = 0.0
    else:
        forget0 = numpy.log(numpy.exp(1.0/
                                       options['arch_spm_include_forgetting'])-1.0)

    # biases are ordered: (1) input gate, (2) time const, (3) input weights
    b = numpy.concatenate([0.0 * numpy.ones((n_hid)),            # input gate
                           forget0 * numpy.ones((n_hid)),        # time const
...
def spm_layer(tparams, state_below, xt, yt, options, prefix='spm', mask=None):
    ...
    def _step(m_, x_, delta_t_input, delta_t_output, o_, h_, gamma_):
        # o_, h_, and gamma_ have dim n_training_examples  X  n_spm
        # delta_t_input has dim n_training_examples

        if (options['arch_spm_include_forgetting']):
            hhat = tensor.exp(-delta_t_input[:,None]*gamma_) * h_
        else:
            hhat = h_
        squashed_hhat = tensor.tanh(hhat)

        # x_ = W * x_t + b
        preact = x_ + tensor.dot(squashed_hhat, tparams[_p(prefix, 'U')]) 
        if (options['arch_spm_input_includes_gamma']):
            lgamma_ = tensor.log(gamma_)
            preact += (tensor.concatenate([gamma_,gamma_,gamma_],axis=1) *
                       tparams[_p(prefix, 'V')][None,:] +
                       tensor.concatenate([lgamma_,lgamma_,lgamma_],axis=1) *
                       tparams[_p(prefix, 'Vlog')][None,:])

        z = tensor.tanh(_slice(preact,2,n_hid))
        if (options['arch_spm_include_input_gate']):
            z *= tensor.nnet.sigmoid(_slice(preact, 0, n_hid))

        h = hhat + z

        if (options['arch_spm_include_forgetting']):
            gamma = tensor.nnet.softplus(_slice(preact,1,n_hid))
            o = tensor.tanh( tensor.exp(-delta_t_output[:,None]*gamma) * h )
        else:
            o = tensor.tanh(h)
...
def train_model(
    ...
    arch_spm_include_forgetting = True, # spm has forgetting (numeric value = initial time scale)
    arch_spm_input_includes_gamma = False, # gamma and log(gamma) are additional inputs to SPM
