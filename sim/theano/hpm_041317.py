...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, y_):
        ...
        # determine memory time constant

        g = (_slice(net, 1, n_hid)).dimshuffle((0,'x',1)) # add index for timescales

        # credit assignment to time scales

        c = tensor.exp(-(g - loggamma) ** 2.0)
