...
def ctgru_layer(tparams, state_below, xt, yt, options, prefix='ctgru', mask=None):
    ...
    def _step(m, x_, delta_t_input, delta_t_output, h_, y_):
        ...
        rho = tensor.exp(-(r.dimshuffle((0,'x',1)) - loggamma) ** 2.0)

        # retrieve memory
        retrieved_y_ = tensor.sum(h_ * rho, axis=1) / tensor.sum(rho, axis=1)
        ...
        if (options['arch_ctgru_ohat_for_sscale']):
            s = _slice(x_, 1, n_hid) + tensor.dot(retrieved_y_, tparams[_p(prefix,'Us')])
        else:
            s = _slice(x_, 1, n_hid) + tensor.dot(y_, tparams[_p(prefix,'Us')])

        # credit assignment to timescales
        sigma = tensor.exp(-(s.dimshuffle((0,'x',1)) - loggamma) ** 2.0)
        sigma = sigma / tensor.sum(sigma,axis=1,keepdims=True)

        # update memory 

	hhat = (1.0 - sigma) * h_ + sigma * q.dimshuffle((0,'x',1))

	# decay hidden according to timescale

        h = hhat * tensor.exp(-gamma * delta_t_output[:,None,None])
