...
4/14/2017 Included absorption idea
          training lambda
...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    """
    n_hid = options['n_hid']
    timescales = options['diffusion_timescales']
    ...
    if (options['arch_hpm_recurrent']):
        U = numpy.concatenate([ortho_weight(n_hid),
                               ortho_weight(n_hid),
                               ortho_weight(n_hid)], axis=1)
        params[_p(prefix, 'U')] = (init_wt_mag / numpy.sqrt(n_hid)) * U 
    ...
    b = numpy.concatenate([numpy.zeros(n_hid,),
                           g0 * numpy.ones(n_hid,),
                           numpy.zeros(n_hid,)], axis=0)
    params[_p(prefix, 'b')] = b.astype(config.floatX)
    ...
    # lambda
    if (options['absorption_timescale'] > 0.0):
        lam = (numpy.log(numpy.ones(n_hid,) / options['absorption_timescale'])
                 + (numpy.random.rand(n_hid,)*4.0-2.0)  )
        params[_p(prefix, 'lambda')] = lam.astype(config.floatX)
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    gamma = (1.0 / numpy_floatX(timescales)).reshape((1, -1, 1)) # relative decays
    loggamma = numpy.log(gamma)
    if (options['absorption_timescale'] > 0.0):
        lam = tensor.exp(tparams[_p(prefix,'lambda')]).dimshuffle(('x','x',0)) 
        # base decay 
    else:
        lam = -1.0 / options['absorption_timescale'] # constant base decay
    gammalam = gamma * lam  # absolute decay rates
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, g_, h_, y_):
        ...
        if (options['arch_hpm_recurrent']):
            net += tensor.dot(y_, tparams[_p(prefix,'U')])
            # U: n_hid X 3n_hid 
        ...
        if options['arch_input_map_constraint'] == 'strong':
            ...
            z = tensor.tanh(_slice(net, 0, n_hid))
            if (options['arch_hpm_gated']):
                z = z * tensor.nnet.sigmoid(_slice(net, 2, n_hid))
        z = z.dimshuffle((0,'x',1)) # add index for timescales

        # determine diffusion rate (relative to lambda) 

        d = (_slice(net, 1, n_hid)).dimshuffle((0,'x',1)) # add index for timescales

        # credit assignment to time scales

        c = tensor.exp(-(d - loggamma) ** 2.0)
	...
	# decay hidden according to timescale
        ...
        diff_decay = tensor.exp(-gammalam * delta_t_output[:,None,None])
        absorp_decay = tensor.exp(-lam * delta_t_output[:,None,None])

        h = (h_ * diff_decay) + ((diff_decay - absorp_decay) * ghat / (1.0 - gamma))
        g = absorp_decay * ghat

        # output is sum of activity across timescales

        y = tensor.tanh(tensor.sum(h,axis=1))
    ...
    rval, updates = theano.scan(_step,
                                sequences=[mask, state_below, xt, yt],
                                outputs_info=[tensor.alloc(numpy_floatX(0.),  # g
                                                           n_examples,
							   n_timescales,
                                                           n_hid),
                                              tensor.alloc(numpy_floatX(0.),  # h
                                                           n_examples,
							   n_timescales,
                                                           n_hid),
                                ...
                                name=_p(prefix, '_layers'),
                                n_steps=nsteps)
    return rval[2], rval[1] # return y, h
...
def train_model(
    ...
    absorption_timescale=-.1, # if negative, then constant, else init value for variable
    diffusion_timescales=2.0**numpy.arange(1,11),
    noise_std=0.,
    use_dropout=False,  
