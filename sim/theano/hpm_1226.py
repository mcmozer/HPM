...
init_wt_mag_output = 0.1      # weights to output
init_wt_mag_recurrent = 0.01  # recurrent connections
...
def init_params(options):
    ...
    if (options['arch_output_fn'] != '1-1'):
       wt_scale = init_wt_mag_output / numpy.sqrt(options['n_hid']+1.0)
       params['U'] = wt_scale * numpy.random.randn(options['n_hid'],
       				  options['n_out']).astype(config.floatX)
       params['b'] = wt_scale * numpy.random.randn(options['n_out'],).astype(config.floatX)
...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    if (options['arch_hpm_recurrent']):
        # initialize recurrent connections (both to HPM inputs and gates)
        if (options['arch_hpm_gated']):
            U = numpy.concatenate([ortho_weight(n_hid),
                                   ortho_weight(n_hid)], axis=1)
        else:
            U = ortho_weight(n_hid) 
        # scale down recurrent weights
        params[_p(prefix, 'U')] = (init_wt_mag_recurrent / numpy.sqrt(n_hid)) * U 

    baserate = 1e-6 * numpy.ones((n_hid))   # DEBUG was 1e-4 before 7/23/16
    ...
    alpha = 0.5 * numpy.ones((n_hid)) # was 1.0 before 12/30/16
    ...
    if (options['arch_hpm_prior_exp']):
        # start with uniform priors -- was random before 12/30/16
        #priorexp = 10.0**(-5.0*numpy.random.rand(n_hid))
        priorexp = 0.0 * numpy.ones((n_hid))
        params[_p(prefix, 'priorexp')] = priorexp.astype(config.floatX)

    if (options['arch_hpm_gamma_scaled_mu']):
        # y = ln(1+exp(net))  net = ln(exp(y)-1)
        # start with exponent of .01 (changed on 12/30/16)
        gamma_exp_for_mu = numpy.log(numpy.exp(.01)-1.0)*numpy.ones((n_hid)) 
                                                                # anti soft-plus
        params[_p(prefix, 'gamma_exp_for_mu')] = (
                                        gamma_exp_for_mu.astype(config.floatX))
    if (options['arch_hpm_gamma_scaled_alpha']):
        # start with exponent of 1
        gamma_exp_for_alpha = numpy.log(numpy.exp(1.0)-1.0)*numpy.ones((n_hid)) 
                                                                # anti soft-plus
        params[_p(prefix, 'gamma_exp_for_alpha')] = (
                                     gamma_exp_for_alpha.astype(config.floatX))

    # output calibration variable -- start around 0.1
    eta = numpy.random.beta(10.0, 90.0, size=(n_hid))
    eta_logit = numpy.log(numpy.exp(eta)-1.0) # anti soft-plus
    params[_p(prefix, 'eta')] = eta_logit.astype(config.floatX) 

    # initialize embedding for sparsity
    if (options['arch_remap_input']):
        # need to add one embedding vector for no input
        params['Wemb'] = numpy.random.beta(1.0, 10.0, size=(options['n_in']+1,options['n_hid']))
        if (options['arch_hpm_gated']):
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    # to enforce shift from short to long time scales with evidence,
    # we bias mu and/or alpha based on gamma
    mu = tensor.nnet.softplus(tparams[_p(prefix, 'mu')]).dimshuffle(('x','x',0))
    if (options['arch_hpm_gamma_scaled_mu']):
        gamma_exp_for_mu = (tensor.nnet.softplus(
               tparams[_p(prefix, 'gamma_exp_for_mu')])).dimshuffle(('x','x',0))
        mu *= gamma ** gamma_exp_for_mu

    alpha = tensor.nnet.softplus(
                            tparams[_p(prefix,'alpha')]).dimshuffle(('x','x',0))
    if (options['arch_hpm_gamma_scaled_alpha']):
        gamma_exp_for_alpha = (tensor.nnet.softplus(
            tparams[_p(prefix, 'gamma_exp_for_alpha')])).dimshuffle(('x','x',0))
        alpha *= gamma ** gamma_exp_for_alpha 
    else:
        alpha *= gamma
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        if (not options['arch_remap_input']):
            ...
            if (options['arch_hpm_recurrent']):
                ...
                # rescaled to [-1,+1] on 1/3/2017
                marginal_intensity_input = 2.0 * (marginal_intensity_input / 
                               (eta + marginal_intensity_input)) - 1.0
                net += tensor.dot(
            ...
            if (options['arch_hpm_gated']):
                ...
                # ungated_event was sigmoid prior to 12/16
                ungated_event = tensor.minimum(1.0,
                                        tensor.nnet.relu(_slice(net, 0, n_hid)))
                event = gate * ungated_event
            else:
                net += state_below
                # event was sigmoid prior to 12/16
                event = tensor.minimum(1.0, tensor.nnet.relu(net))
	...
	# predict next event conditioned on timescale
        ...
        # event versus ghost event?
        # rescaled to [-1,+1] range on 1/3/2017
        marginal_intensity_output = 2.0 * (marginal_intensity_output / 
                               (eta + marginal_intensity_output)) - 1.0
...
def print_weights(tparams,options):
    ...
    if (options['encoder'] == 'hpm'):
        if (options['arch_hpm_gamma_scaled_mu']):
            print('hpm_gamma_exp_for_mu:\n',
                 numpy.log(1.0+numpy.exp(print_params['hpm_gamma_exp_for_mu'])))
        if (options['arch_hpm_gamma_scaled_alpha']):
            print('hpm_gamma_exp_for_alpha:\n',
              numpy.log(1.0+numpy.exp(print_params['hpm_gamma_exp_for_alpha'])))
        print('hpm_mu:\n',numpy.log(1.0+numpy.exp(print_params['hpm_mu'])))
        print('hpm_alpha:\n',numpy.log(
                                      1.0+numpy.exp(print_params['hpm_alpha'])))
        print('hpm_eta:\n',1.0/(1.0 + numpy.exp(-print_params['hpm_eta'])))
        if (options['arch_hpm_prior_exp']):
...
def train_model(
    ...
    arch_hpm_gamma_scaled_mu=True, # scale mu by exponentiated gamma
    arch_hpm_gamma_scaled_alpha=True, # scale alpha by exponentiated gamma
