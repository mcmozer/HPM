...
3/11/2017 This version allows parallel HP at multiple scales
...
def init_params(options):
    ...
    if options['arch_input_map_constraint'] != 'strong':
        if options['arch_input_map_constraint'] == 'none':
            ...
            if options['arch_input_map_constraint'] == 'weak':
                ...
                w = options['arch_input_map_constraint']
            # diagonal weights
            if (options['signed_in']): # 2 inputs per hidden
                # give negative value half the weight as positive
                wemb = w * numpy.concatenate((numpy.zeros((1,options['n_hid'])),
                                          .5*numpy.identity(options['n_hid']),
                                          numpy.identity(options['n_hid'])), axis=0)
            else: # 1 input per hidden
                wemb = w * numpy.concatenate((numpy.zeros((1,options['n_hid'])),
                                          numpy.identity(options['n_hid'])), axis=0)
        params['Wemb'] = wemb.astype(config.floatX)
    ...
    if (options['arch_output_fn'] != '1-1'):
        ...
        # force output mapping constraint if input mapping constraint
        if (options['arch_input_map_constraint'] != 'none'):
            print('WARNING: output map constraint applied too!')
            if options['arch_input_map_constraint'] == 'weak':
                w = 2.0
            else:
                w = options['arch_input_map_constraint']
            params['U'] = (numpy.identity(options['n_hid'])
                                                    * w).astype(config.floatX)
        else:
...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    # INITIALIZE MU
    baserate = 1e-3 * numpy.ones((n_hid))   
    mu = -numpy.log(1-baserate) 
    params[_p(prefix, 'mu')] = numpy.log(
                      numpy.exp(mu.astype(config.floatX))-1.0) # anti soft-plus

    # INITIALIZE MU EXPONENT
    ahme = options['arch_hpm_mu_exp']
    if not (ahme in {0.0, 1.0}):
        if ahme < 0.0:
            mu_exp_logit = numpy.random.randn((n_hid))
        else:
            mu_exp_logit = numpy.ones((n_hid)) * numpy.log(ahme / (1.0 - ahme))
        params[_p(prefix,'mu_exp_logit')] = mu_exp_logit.astype(config.floatX)

    # INITIALIZE ALPHA
    alpha = 0.3 * numpy.random.randn((n_hid))
    params[_p(prefix, 'alpha')] = alpha.astype(config.floatX)

    # INITIALIZE ALPHA EXPONENT
    ahae = options['arch_hpm_alpha_exp']
    if not (ahae in {0.0, 1.0}):
        if ahae < 0.0:
            alpha_exp_logit = numpy.random.randn((n_hid))
        else:
            alpha_exp_logit = numpy.ones((n_hid)) * numpy.log(ahae / (1.0 - ahae))
        params[_p(prefix,'alpha_exp_logit')] = alpha_exp_logit.astype(config.floatX)

    # INITIALIZE PRIOR EXPONENT
    ahpe = options['arch_hpm_prior_exp']
    if not (ahpe in {0.0, 1.0}):
        if ahpe < 0.0:
            prior_exp_logit = numpy.random.randn((n_hid))
        else:
            prior_exp_logit = numpy.ones((n_hid)) * numpy.log(ahpe / (1.0 - ahpe))
        params[_p(prefix,'prior_exp_logit')] = prior_exp_logit.astype(config.floatX)


    if options['arch_hpm_scaled_gamma']:
        log_gamma_scale = 3.0 * numpy.random.randn((n_hid))
        params[_p(prefix, 'log_gamma_scale')] = log_gamma_scale.astype(config.floatX)

    # remapping of the embedding 
    if options['arch_input_map_constraint'] != 'strong':
        if (options['arch_hpm_gated']):
            b = numpy.zeros(2*n_hid,) 
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    timescales = options['timescales']
    if (options['arch_hpm_scaled_gamma']):
        gamma = ( tensor.exp(tparams[_p(prefix,'log_gamma_scale')]).dimshuffle('x','x',0) /
    ...
    # determine mu (base_rate) by timescale
    base_rate = tensor.nnet.softplus(
                                  tparams[_p(prefix, 'mu')]).dimshuffle(('x','x',0))
    if options['arch_hpm_mu_exp'] == 1.0:
        base_rate *= gamma
    elif options['arch_hpm_mu_exp'] != 0.0:
        mu_exp = tensor.nnet.sigmoid(
                         tparams[_p(prefix,'mu_exp_logit')]).dimshuffle(('x','x',0))
        base_rate *= gamma ** mu_exp


    # determine alpha by timescale
    alpha = tparams[_p(prefix,'alpha')].dimshuffle(('x','x',0))
    if options['arch_hpm_alpha_exp'] == 1.0:
        alpha *= gamma
    elif options['arch_hpm_alpha_exp'] != 0.0:
        alpha_exp = tensor.nnet.sigmoid(
                      tparams[_p(prefix,'alpha_exp_logit')]).dimshuffle(('x','x',0))
        alpha *= gamma ** alpha_exp

    # determine prior by timescale
    if options['arch_hpm_prior_exp'] == 1.0:
        prior = gamma
    elif options['arch_hpm_prior_exp'] == 0.0:
        prior = numpy.ones((n_timescales))
    else:
        prior_exp = tensor.nnet.sigmoid(
                      tparams[_p(prefix,'prior_exp_logit')]).dimshuffle(('x','x',0))
        prior = gamma ** prior_exp
    prior = prior / tensor.sum(prior, axis=1)
    ...
    # h has dimensions: n_training_examples X n_timescales X n_hid
    def _sum_across_timescale(h,br):
        hbr = h+tensor.switch(tensor.lt(h,0.0),-br,br)
        return tensor.sum(hbr,axis=1) # return dim: n_training_examples X n_hid

    def _decay_intensity(intensity, delta_t):
        # remember that base_rate is subtracted from all intensities
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        h  = _decay_intensity(h_, delta_t_input[:,None,None])

        if options['arch_input_map_constraint'] == 'strong':
            # in order to behave as an ordinary HP
            # don't allow rescale input when passing 0/1 inputs through
            signed_event = state_below.dimshuffle((0,'x',1)) 
        else:
            net = tparams[_p(prefix,'b')]
            if (options['arch_hpm_recurrent']):
                marginal_intensity_input = _sum_across_timescale(h,base_rate)
                marginal_intensity_input = theano.tensor.tanh(
                                               marginal_intensity_input)
                net += tensor.dot(
            ...
            if (options['arch_hpm_gated']):
                ...
                signed_event = gate * tensor.tanh(_slice(net, 0, n_hid))
            else:
                net += state_below
                signed_event = tensor.tanh(net)

            # dimensions: n_training_examples X n_timescales X n_hid
            signed_event = signed_event.dimshuffle((0,'x',1))

        # reallocate event to timescales
        c = _timescale_posterior(abs(h) + base_rate, prior) 

        # update intensity 
	h += alpha * signed_event * c

	# clear out updates after end of sequence
        h = m[:, None, None] * h + (1. - m)[:, None, None] * h_

	# predict next event conditioned on timescale
        hhat = _decay_intensity(h, delta_t_output[:,None,None])

        # expectation of intensity: has dimensions n_training_examples X n_hid
        marginal_intensity_output = _sum_across_timescale(hhat, base_rate)
        if (options['arch_output_fn'] == '1-1'):
            marginal_intensity_output = abs(marginal_intensity_output)
            # DEBUG: may need to add base_rate here
        else:
...
def print_weights(tparams,options):
    ...
    if (options['encoder'] == 'hpm'):
        ...
        if not (options['arch_hpm_alpha_exp'] in {0.0, 1.0}):
            print('hpm_alpha_exp:\n', 
                                1.0/(1.0+numpy.exp(-print_params['hpm_alpha_exp'])))
        if not (options['arch_hpm_prior_exp'] in {0.0, 1.0}):
            print('hpm_prior_exp:\n', 
                                1.0/(1.0+numpy.exp(-print_params['hpm_prior_exp'])))
        if not (options['arch_hpm_mu_exp'] in {0.0, 1.0}):
            print('hpm_mu_exp:\n', 1.0/(1.0+numpy.exp(-print_params['hpm_mu_exp'])))
    if (options['arch_output_fn'] != '1-1'):
        print('U:\n',print_params['U'])
        print('b:\n',print_params['b'])
    if options['arch_input_map_constraint'] != 'strong':
...
def recode_value_as_index(data,n_in):
    new_data = [[abs(data[n][j])+n_in*(data[n][j]>0) 
...
def train_model(
    ...
    arch_hpm_prior_exp=1.0, # exponent on gamma for prior
    arch_hpm_alpha_exp=1.0, # exponent on gamma for alpha
    arch_hpm_mu_exp=1.0, # exponent on gamma for mu
    ...
    n_hid=40,  # number of HPM hidden units; ignored if arch_input_map_constraint!='none'
    ...
    n_in = numpy.max(list(chain(*train[X])))
    if (model_options['signed_in']):
       train = (recode_value_as_index(train[X],n_in), train[XT], train[Y], train[YT])
       valid = (recode_value_as_index(valid[X],n_in), valid[XT], valid[Y], valid[YT])
       test = (recode_value_as_index(test[X],n_in), test[XT], test[Y], test[YT])
       n_in *= 2

    model_options['n_in'] = n_in
    ...
    # input max
    if (model_options['arch_input_map_constraint']=='strong' and
            (model_options['arch_hpm_gated'] or model_options['arch_hpm_recurrent'])):
        print('Error: recurrent and gated arch only if input remapped');
        quit()
    if not model_options['signed_in']: # unsigned inputs
        ...
        if (model_options['arch_input_map_constraint'] != 'none' 
                and model_options['arch_output_fn']=='1-1'
                and model_options['n_out']*2 != model_options['n_in']):
            print('Error: # input and output alternatives must match');
            quit()
        if (model_options['arch_input_map_constraint'] != 'none' 
                    and model_options['n_hid'] != model_options['n_out']):
           model_options['n_hid'] = model_options['n_out']
           print('Warning: forcing n_hid = n_out');
