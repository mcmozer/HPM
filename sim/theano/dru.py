'''
3/22/2017 quickie implementation of Patrice Simard's DRU
'''
...
def param_init_dru(options, params, prefix='dru'):
    """
    Init the DRU parameter:
    ...
    U = numpy.concatenate([ortho_weight(n_hid),
                           ortho_weight(n_hid),
                           ortho_weight(n_hid)], axis=1)
    # augment DRU input feature weights to include delta_t values
    if (options['arch_dru_include_delta_t']):
       U = numpy.concatenate([U, 
                      .01 * numpy.random.randn(2,3 * n_hid)],axis=0)

    params[_p(prefix, 'U')] = U
    internal_state_weight = numpy.zeros((3 * n_hid))
    params[_p(prefix,'internal_state_weight')] = internal_state_weight.astype(config.floatX)
...
def dru_layer(tparams, state_below, xt, yt, options, prefix='dru', mask=None):
    ...
    def _step(m_, x_, delta_t_input, delta_t_output, h_, c_):
        # h_ has dim n_training_examples  X  n_dru
        # delta_t_input has dim n_training_examples

        # include input and output delta_t values as DRU input features
        if (options['arch_dru_include_delta_t']):
        ...
        preact = (tensor.dot(h_aug, tparams[_p(prefix, 'U')]) 
                  + x_ 
                  + tensor.concatenate([c_, c_, c_], axis=1) 
                            * tparams[_p(prefix, 'internal_state_weight')])

        yminus = tensor.nnet.relu(-_slice(preact, 0, options['n_hid']))
        yplus = tensor.nnet.relu(_slice(preact, 1, options['n_hid']))
        y = yplus - yminus
        ...
        if (options['arch_dru_include_output_gate']):
...
def train_model(
    ...
    arch_dru_include_delta_t = True, #include delta_t values in dru input 
    arch_dru_include_output_gate = False, # DRU has output gate 
    ...
    encoder='dru',  # dru or hpm
    saveto='weights/dru_model.npz',  # The best model will be saved there
    ...
    global layers
    if model_options['encoder'] == 'dru':
        layers = {'dru': (param_init_dru, dru_layer)}
    else:
