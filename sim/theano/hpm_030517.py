...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    # changed on 3/4/17 from mu = 1.0e-7
    mu = 1.0e-3 * numpy.ones((n_timescales,n_hid))
    params[_p(prefix, 'mu')] = numpy.log(
                      numpy.exp(mu.astype(config.floatX))-1.0) # anti soft-plus

    # alpha: n_timescales X n_hidden
    if options['arch_hpm_alpha_constraint'] == 'none':
        alpha = 0.2 * numpy.random.randn(n_timescales,n_hid)
    elif options['arch_hpm_alpha_constraint'] == 'weak':
        alpha = numpy.outer(1./timescales, 0.5 * numpy.ones(n_hid))
    else: # strong init -- one alpha per time scale
        alpha = 0.5 * numpy.ones(n_hid)
    ...
    if (options['arch_hpm_prior_exp']):
        ...
        if (options['arch_hpm_prior_gaussian']):
            ...
            # changed on 3/4/17 from numpy.random.randn
            priors = numpy.ones((n_hid,n_timescales)) 
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    if options['arch_hpm_alpha_constraint'] == 'strong':
        # one alpha per hidden
        alpha = tparams[_p(prefix,'alpha')].dimshuffle(('x','x',0)) * gamma
    else:
        # one alpha per hidden and per time scale
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        if options['arch_input_map_constraint'] == 'strong':
            ...
            if (options['arch_hpm_recurrent']):
                marginal_intensity_input = _marginalize_timescale(
                                   h+tensor.switch(tensor.lt(h,0.0),
                                              -stationary_rate,stationary_rate), c_)
        ...
        unsigned_event = abs(signed_event)
        # credit assignment across timescales
        c = (unsigned_event * 
                _timescale_posterior(abs(h) + stationary_rate,c_) 
	...
	# predict next event conditioned on timescale
        ...
        marginal_intensity_output = _marginalize_timescale(
                                      hhat+tensor.switch(tensor.lt(hhat,0.0),
                                              -stationary_rate,stationary_rate), c)
        if (options['arch_output_fn'] == '1-1'):
            marginal_intensity_output = abs(marginal_intensity_output)
            # DEBUG: may need to add stationary_rate here
        else:
...
def train_model(
    ...
    arch_hpm_alpha_constraint='none', # constraint on alphas ('none','strong','weak')
