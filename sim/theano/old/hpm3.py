...
def init_params(options):
    ...
    #   randn = numpy.random.randn(options['n_in'],
    #                              options['n_hpu'])
...
def train_model(
    ...
    if (not model_options['arch_remap_input'] 
       ...
       quit()
    if (not model_options['arch_remap_input']):
       model_options['n_hpu'] = model_options['n_in']-1
    if (not model_options['arch_remap_output']):
       model_options['n_hpu'] = model_options['n_out']-1

    print('Building model with',model_options['n_in']-1,'inputs,',
          model_options['n_hpu'],'hidden, and',model_options['n_out']-1,
	  'outputs')
...
if __name__ == '__main__':
    ...
    train_model(arch_remap_output=False, arch_remap_input=True, n_hpu=3)
