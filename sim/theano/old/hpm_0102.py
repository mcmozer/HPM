'''
2/10/17 Replaced this version with one that gave complete freedom to setting
of priors, alpha, mu. This version has the constraints on alpha, mu, and priors as a function of gamma. 
...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    alpha0 = numpy.random.randn((n_hid))*.2
    params[_p(prefix, 'alpha0')] = alpha0.astype(config.floatX)
    ...
    if (options['arch_hpm_gamma_scaled_mu']):
        gamma_exp_for_mu = numpy.log(numpy.exp(1e-4)-1.0)*numpy.ones((n_hid)) 
                                                                # anti soft-plus
        params[_p(prefix, 'gamma_exp_for_mu')] = (
                                        gamma_exp_for_mu.astype(config.floatX))
    if (options['arch_hpm_gamma_scaled_alpha']):
        gamma_exp_for_alpha = numpy.ones((n_hid))
        params[_p(prefix, 'gamma_exp_for_alpha')] = gamma_exp_for_alpha.astype(config.floatX)
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    stationary_rate = tensor.nnet.softplus(tparams[_p(prefix, 'mu')]).dimshuffle(('x','x',0))
    if (options['arch_hpm_gamma_scaled_mu']):
    ...
    alpha0 = tparams[_p(prefix,'alpha0')].dimshuffle(('x','x',0))
    if (options['arch_hpm_gamma_scaled_alpha']):
        gamma_exp_for_alpha = tparams[_p(prefix, 'gamma_exp_for_alpha')].dimshuffle(('x','x',0))
...
def train_model(
    ...
    arch_hpm_gamma_scaled_mu=False, # scale mu by exponentiated gamma
    arch_hpm_gamma_scaled_alpha=False, # scale mu by exponentiated gamma
