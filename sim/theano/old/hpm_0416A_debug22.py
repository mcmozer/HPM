...
def param_init_hpm(options, params, prefix='hpm'):
   ...
   #                   numpy.exp(mu.astype(config.floatX))-1.0) # anti soft-plus
    ...
    if (options['arch_hpm_gamma_scaled_mu']):
        gamma_exp_for_mu = numpy.log(numpy.exp(1.0)-1.0)*numpy.ones((n_hid)) 
                                                                # anti soft-plus
...
#        if (options['arch_hpm_gated']):
#            b = .05 * numpy.random.randn(2*n_hid,) - 1.5
...
#        alpha *= gamma ** gamma_exp_for_alpha 
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        c = c_ + 1
        h = h_ + state_below.dimshuffle((0,'x',1))
        marginal_intensity_output = tensor.sum(h / c, axis=1)
...
#            if (options['arch_hpm_recurrent']):
#                marginal_intensity_input = _marginalize_timescale(z, c_)
#                net += tensor.dot(
...
#        #marginal_intensity_output = _marginalize_timescale(1-zhat, c)
    ...
    c0 = numpy_floatX(1.)/numpy_floatX(n_timescales)
...
def pred_error(f_corr, f_cost, prepare_data, data, iterator, options, f_pred_prob, f_proj, verbose=False):
    """
...
def train_model(
    ...
    f_pred_prob = theano.function([x, xt, yt, mask], pred_prob, name='f_pred_prob', mode='DebugMode')
    ...
    f_grad = theano.function([x, xt, y, yt, mask], grads, name='f_grad', mode='DebugMode')

    # DEBUG output functions
    #f_proj = theano.function([x, xt, yt, mask], proj, name='f_proj')
    f_proj = theano.function([x, xt, yt, mask], proj, name='f_proj', mode='DebugMode')
    ...
    try:
        for eidx in range(1,max_epochs+1):
            ...
            for ix, train_index in kf:
                ...
                pred_prob = f_pred_prob(x,xt,yt,mask)
                print('proj (#steps X # examples X #hid):\n', proj)
                print('pred_prob (#steps X # examples X #out):\n', pred_prob)
                print('shape pred_prob ',pred_prob.shape,'proj', proj.shape)
                g = f_grad(x,xt,y,yt,mask)
                print('grads\n',g)
                print('tparams\n',tparams)
		...
		train_err, train_cost = pred_error(f_corr,f_cost, 
                                            prepare_data,train,kf,model_options,f_pred_prob, f_proj)
		valid_err, valid_cost = pred_error(f_corr, f_cost, 
                                      prepare_data,valid,kf_valid,model_options,f_pred_prob, f_proj)
		test_err, test_cost = pred_error(f_corr,f_cost, 
                                        prepare_data,test,kf_test,model_options,f_pred_prob, f_proj)
    ...
    train_err, train_cost = pred_error(f_corr,f_cost,prepare_data,
                                            train,kf_train_sorted,model_options, f_pred_prob, f_proj)
    valid_err, valid_cost = pred_error(f_corr,f_cost,prepare_data, 
                                                   valid,kf_valid,model_options, f_pred_prob, f_proj)
    test_err, test_cost  = pred_error(f_corr,f_cost,prepare_data, 
                                                     test,kf_test,model_options, f_pred_prob, f_proj)
