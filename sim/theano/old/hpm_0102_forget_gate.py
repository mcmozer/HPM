...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    if (options['arch_hpm_recurrent']):
        if (options['arch_hpm_include_input_gate']):
            if (options['arch_hpm_include_forget_gate']):
    ...
    if (options['arch_hpm_prior_exp']):
        ...
        priorexp = -numpy.random.rand((n_hid))
        ...
        if (options['arch_hpm_prior_gaussian']):
            ...
            mean_log_prior = numpy.mean(numpy.log(1.0 / timescales)) * numpy.ones((n_hid))
            std_log_prior = 2.0 * numpy.std(numpy.log(1.0 / timescales)) * numpy.ones((n_hid))
            ...
            priors = numpy.random.randn(n_hid,n_timescales)
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    stationary_rate = tensor.nnet.softplus(tparams[_p(prefix, 'mu')]).dimshuffle(('x',0,1))
    alpha = tparams[_p(prefix,'alpha')].dimshuffle(('x',0,1))
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        if (not options['arch_remap_input']):
            signed_event = state_below
        else:
            net = tparams[_p(prefix,'b')]
            if (options['arch_hpm_recurrent']):
                marginal_intensity_input = _marginalize_timescale(h + stationary_rate, c_)
                marginal_intensity_input = theano.tensor.tanh(marginal_intensity_input)
                net += tensor.dot(
            ...
            if (options['arch_hpm_include_input_gate']):
                ...
                # W:           n_hid X K*n_hid    where K = 2 or 3 depending on forget_gate
                # b:                   K*n_hid [broadcasting is R to L]
                net += tensor.dot(state_below, tparams[_p(prefix, 'W')])
                input_gate  = tensor.nnet.sigmoid(_slice(net, 1, n_hid))
                forget_gate  = tensor.nnet.sigmoid(_slice(net, 2, n_hid))
                signed_event = input_gate * tensor.tanh(_slice(net, 0, n_hid))
            else:
                net += state_below
	...
	signed_event = signed_event.dimshuffle((0,'x',1))
        ...
        c = (unsigned_event * 
                _timescale_posterior(tensor.nnet.relu(h) + stationary_rate,c_) 
             + (1.0-unsigned_event) * c_)

        # update intensity 
        if (options['arch_hpm_include_forget_gate']):
            h = h * forget_gate.dimshuffle((0,'x',1)) + alpha * signed_event
            # h dimensions: n_training_examples  X  n_timescales  X  n_hid
            # forget_gate dimensions:  n_training_examples X n_hid
        else:
            h += alpha * signed_event
	...
	# predict next event conditioned on timescale
        ...
        marginal_intensity_output = _marginalize_timescale(hhat + stationary_rate, c)
    ...
    if (options['arch_hpm_prior_exp']):
        ...
        if (options['arch_hpm_prior_gaussian']):
            ...
            mlp = tparams[_p(prefix,'mean_log_prior')].dimshuffle(('x','x',0))
            slp = tparams[_p(prefix,'std_log_prior')].dimshuffle(('x','x',0))
            c0 = tensor.exp(-(g0 - mlp)**2.0 / 2.0 / slp**2.0)
            c0 = c0 / tensor.sum(c0, axis=1)
        else:
            # take softmax on axis=1 (timescales)
            c0 = tensor.nnet.softmax(tparams[_p(prefix,'priors')])
...
def build_model(tparams, options):
    ...
    def _cost_step_norm(pred_prob_step, y_step):
        ...
        out_prob_step = tensor.switch(tensor.eq(y_step, 0), 1.0, 
                             pred_prob_step[tensor.arange(n_examples),y_step-1])
        tgt_prob_step = out_prob_step
        ...
        return tgt_prob_step, out_prob_step, corr_step
    ...
    def _cost_step_tgt(pred_prob_step, y_step):
        ...
        tgt_prob_step = tensor.switch(tensor.eq(y_step, 0), 1.0, 
                    tensor.switch(tensor.gt(y_step, 0), out_prob_step, 1.0-out_prob_step))
    ...
    (tgt_prob, out_prob, corr), updates = theano.scan(cost_fn,
    ...
    return use_noise, x, xt, y, yt, mask, pred_prob, corr, cost, proj, h, c, tgt_prob, out_prob
...
def pred_error(f_corr, f_cost, f_out_prob, prepare_data, data, iterator, options, verbose=False):
    """
	...
	cost += f_cost(x, xt, y, yt, mask) 
        ...
        pred = f_out_prob(x, xt, y, yt, mask)
        mask_ravel_bool = (mask.ravel()).astype(bool)
        predicted = numpy.append(predicted, pred.ravel()[mask_ravel_bool])


        target = numpy.append(target, y.ravel()[mask_ravel_bool] > 0.)
...
def train_model(
    ...
    arch_hpm_include_input_gate=False, # include gate on input and forget
    arch_hpm_include_forget_gate=False, # include gate on input and forget
    ...
    if (not model_options['arch_remap_input'] and
            (model_options['arch_hpm_include_input_gate'] or model_options['arch_hpm_recurrent'])):
    ...
    (use_noise, x, xt, y, yt, mask, pred_prob, corr, cost, 
        proj, h, c, tgt_prob, out_prob) = build_model(tparams, model_options)
    ...
    f_out_prob = theano.function([x, xt, y, yt, mask],
                                 out_prob, name='f_out_prob')
