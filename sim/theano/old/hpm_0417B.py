...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
	...
	if (options['arch_hpm_recurrent']):
            marginal_intensity_input = 1.0 - _marginalize_timescale(tensor.exp(-(h + mu)), c)
	...
	# predict next event conditioned on timescale
        ...
        # expectation of squashed intensity 
        marginal_intensity_output = 1.0 - _marginalize_timescale(tensor.exp(-(hhat + mu)), chat)
