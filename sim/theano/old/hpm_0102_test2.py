'''
1/17/2017 THIS VERSION OF CODE has an alpha and mu for each time scale
1/17/2017 Changed minimum probability for log likelihood
          cost = max(log(prob_out),MIN_PROB)
1/24/2017 switched from cross entropy on softmax(net_target) 
          on logistic(net_target - max(net_nontarget))
          or logistic(net_target - max(net_all_nontargets_and_target))
          Changing objective function made worse if anything
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    if (options['arch_hpm_prior_exp']):
        ...
        if (options['arch_hpm_prior_gaussian']):
            ...
            #c0 = tensor.exp(unnorm_priors - tensor.max(unnorm_priors, axis=0, keepdims=True))
            #c0 = c0 / tensor.sum(c0, axis=0, keepdims=True)
            #c0 = tensor.nnet.sigmoid(tparams[_p(prefix,'priors')]).dimshuffle(('x',0,1))
            #c0 = c0 / tensor.sum(c0, axis=1)
...
def build_model(tparams, options):
    ...
    def _step(proj_step):
        if (options['arch_output_fn'] == 'softmax'):
            pred_net_step = tensor.dot(proj_step, tparams['U']) + tparams['b']
            pred_prob_step = tensor.nnet.softmax(pred_net_step)
        elif (options['arch_output_fn'] == 'logistic'):
            pred_net_step = tensor.dot(proj_step, tparams['U']) + tparams['b']
            pred_prob_step = tensor.nnet.sigmoid(pred_net_step)
        else: # '1-1'
            pred_net_step = proj_step
            pred_prob_step = (proj_step+1.0e-6) / tensor.sum(proj_step+1.0e-6,axis=1,keepdims=True)
        return pred_prob_step, pred_net_step
        # pred_{prob,net}_step should have dim n_examples X n_outputs
        # pred_{prob,net} has dim n_timesteps x n_examples x n_outputs
 
    (pred_prob, pred_net), updates = theano.scan(_step,
                                       sequences=proj,
				       outputs_info=None,
				       non_sequences=None,
       				       n_steps=n_timesteps)

    def _cost_step_norm(pred_net_step, y_step):
        pred_net_step_without_tgt = theano.tensor.set_subtensor(
                 pred_net_step[tensor.arange(n_examples), 
                                tensor.maximum(0,y_step-1)] , -1.0e90)
        best_nontgt_net_step = tensor.max(pred_net_step_without_tgt,axis=1) 
        
        # tgt_prob_step should have dim n_examples 
        tgt_prob_step = tensor.switch(tensor.eq(y_step, 0), 1.0, 
                         tensor.nnet.sigmoid(
                             pred_net_step[tensor.arange(n_examples),y_step-1] -
                             best_nontgt_net_step))

        pred_ix_step = tensor.argmax(pred_net_step,axis=1) + 1
    ...
    if (options['signed_out']):
        num_alt = 2
        (tgt_prob, corr), updates = theano.scan(_cost_step_tgt,
        ...
        (tgt_prob, corr), updates = theano.scan(_cost_step_norm,
                                            sequences=[pred_net, y],
                                            outputs_info=None,
                                            non_sequences=None,
                                            n_steps=n_timesteps)
...
def train_model(
    ...
    arch_hpm_prior_exp=True, # include exponent on gamma for timescale prior
