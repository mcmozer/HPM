...
def init_params(options):
    ...
    if (options['arch_remap_input']):
        ...
        embedding_mask = numpy.ones((options['n_in']+1,options['n_hid']))

        print('DEBUG: constrained mapping')
        if (1): # DEBUG ********* (options['arch_diag_input']):
           if (options['signed_in']):
              for h in range(randn.shape[1]):
                 for i in range(randn.shape[0]):
                    if (i == h*2+1):
                       randn[i][h] = -numpy.abs(randn[i][h])
                    elif (i == h*2+2):
                       randn[i][h] = numpy.abs(randn[i][h])
                    else:
                       randn[i][h] = 0.0
                       embedding_mask[i][h] = 0.0
           else:
              for h in range(randn.shape[1]):
                 for i in range(randn.shape[0]):
                    if (i != h+1):
...
def build_model(tparams, options):
    ...
    if (options['arch_remap_input']):
       emb = tparams['Wemb']
       if (1): # CONSTRAINED MAPPING
           emb *= embedding_mask
       emb = emb[x.flatten()].reshape([n_timesteps,
						   n_examples,
						   options['n_hid']])
    else:
