...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    """
    n_hid = options['n_hid']
    n_timescales = options['n_timescales']
    ...
    # different timescales for each hidden
    #ts = 10.0 ** (numpy.random.rand(n_timescales,n_hid)*4.0-1.0)
    # same timescales for all hidden
    ts = 10.0 ** (numpy.random.rand(n_timescales)*4.0-1.0)
    params[_p(prefix,'logts')]= numpy.log(ts).astype(config.floatX)
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    n_timescales = options['n_timescales']
    # same timescales for all hidden
    gamma = (1.0 / tensor.exp(tparams[_p(prefix, 'logts')])).dimshuffle(('x',0,'x'))
    # different timescales for each hidden
    #gamma = (1.0 / tensor.exp(tparams[_p(prefix, 'logts')])).dimshuffle(('x',0,1))
	...
	# credit assignment across timescales
        #if (options['arch_hpm_event_lik']=='e'): # expectation
        #    c = _timescale_posterior((1.0-event)+event*(h+mu),c)
        #elif (options['arch_hpm_event_lik']=='b'): # bernoulli
...
def train_model(
    ...
    (use_noise, x, xt, y, yt, mask, 
           pred_prob, corr, cost) = build_model(tparams, model_options)
