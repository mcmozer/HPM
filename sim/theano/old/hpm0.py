...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    :see: init_params
    """
    n_hpu = options['dim_proj']

    if (options['arch_recurrent']):
       U = ortho_weight(n_hpu)
...
def build_model(tparams, options):
    ...
    if (options['arch_remap_input']):
       ...
       Wemb = theano.shared( numpy.concatenate(([[0,0,0]],
                       numpy.identity(options['dim_proj'],dtype=config.floatX)),
                       axis=0), name='Wemb')
    ...
    # proj has dim n_timesteps X n_examples X dim_proj
    ...
    def _cost_step(pred_prob_step, y_step):
       tgt_prob_step = tensor.gt(y_step, 0) * pred_prob_step[tensor.arange(n_examples),y_step]/(1.0-pred_prob_step[tensor.arange(n_examples),0])  + (1.0-tensor.gt(y_step,0))
    ...
    #cost = -tensor.log(tgt_prob+off).mean(axis=None)
    #cost = -tensor.log(tgt_prob.clip(off, 1.0)).mean(axis=None)
...
def pred_error(f_pred, prepare_data, data, iterator, verbose=False):
	...
	ct += (y > 0).sum()
    err = 1.0 - numpy_floatX(correct_ct) / numpy_floatX(ct)

    return err

def train_model(
    arch_remap_input=False, # should input layer be remapped to hidden or 1-1?
    arch_remap_output=False, # should hidden layer be remapped to a softmax layer or 1-1 to output?
    arch_recurrent=False, # include recurrent connections between memory units
    dim_proj=40,  # number of HPM hidden units; ignored if arch_remap_input True
    ...
    print('Loading data')
    train, valid, test = load_data(valid_portion=0.05, maxlen=maxlen)
    if test_size > 0:
    ...
    # if 1-1 mapping between inputs and hidden, then dim_proj is forced to 
    # input max
    if (model_options['arch_remap_input'] and model_options['arch_remap_output']
    ...
    if (not model_options['arch_remap_input']):
       model_options['dim_proj'] = model_options['xdim']-1
    if (not model_options['arch_remap_output']):
       model_options['dim_proj'] = model_options['ydim']-1
    ...
    if validFreq == -1:
        validFreq = len(train[X]) // batch_size
    if saveFreq == -1:
        saveFreq = len(train[X]) // batch_size
    ...
    try:
        for eidx in range(max_epochs):
		    ...
		    print('hpm_mu ',numpy.exp(print_params['hpm_logmu']))
		    print('hpm_alpha ',numpy.exp(print_params['hpm_logalpha']))
		    print('hpm_prior_exp ',numpy.exp(print_params['hpm_logpriorexp']))
                ...
                if numpy.mod(uidx, validFreq) == 0:
                    ...
                    print( ('Accuracy: Train ', 1-train_err, 'Valid ', 1-valid_err,
                           'Test ', 1-test_err) )
            ...
            print('Seen %d examples' % n_examples)
    ...
    print( 'Accuracy: Train ', 1-train_err, 'Valid ', 1-valid_err, 'Test ', 1-test_err )
    if saveto:
