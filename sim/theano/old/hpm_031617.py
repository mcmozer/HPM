...
3/16/2017 This version restricts intensity to be nonnegative as original model
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    def _sum_across_timescale(h,br):
        return tensor.sum(h+br,axis=1) # return dim: n_training_examples X n_hid
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        if options['arch_input_map_constraint'] == 'strong':
            ...
            event = state_below.dimshuffle((0,'x',1)) 
        else:
            ...
            if (options['arch_hpm_gated']):
                ...
                event = gate * tensor.nnet.sigmoid(_slice(net, 0, n_hid))
            else:
                net += state_below
        ...
        c = _timescale_posterior(h + base_rate, prior) 
