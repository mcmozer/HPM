...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    alpha0 = numpy.random.randn((n_hid)) # logit values
    ...
    if (options['arch_hpm_gamma_scaled_mu']):
        print('***** ERROR: should not use arch_hpm_gamma_scaled_mu\n')
    ...
    if (options['arch_hpm_gamma_scaled_alpha']):
        gamma_exp_for_alpha = -numpy.log(1.0/0.90 - 1.0)*numpy.ones((n_hid)) 
                                                                # anti logistic
        params[_p(prefix, 'gamma_exp_for_alpha')] = (
                                     gamma_exp_for_alpha.astype(config.floatX))

    eta = numpy.log(numpy.exp(1.0)-1.0)*numpy.ones((n_hid)) 
    # output calibration
    params[_p(prefix, 'eta')] = eta.astype(config.floatX) # anti soft-plus

    #alpha_gamma_ratio = numpy.random.randn((n_hid)) # anti logistic
    #params[_p(prefix, 'alpha_gamma_ratio')] = (
    #                                 alpha_gamma_ratio.astype(config.floatX))
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
	...
	# predict next event conditioned on timescale
        hhat = _event_prob(h, delta_t_output[:,None,None])
...
def train_model(
    ...
    arch_hpm_gamma_scaled_alpha=False, # scale alpha by exponentiated gamma
