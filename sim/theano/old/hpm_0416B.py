...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
	...
	# credit assignment across timescales
        cneg = _timescale_posterior(z, c_)
        cpos = _timescale_posterior(1.0 - z, c_)
        c = event * cpos + (1.0-event) * cneg

	# previous scheme to handle Bayes update of timescale distribution
