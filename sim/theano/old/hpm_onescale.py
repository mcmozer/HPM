...
def param_init_lstm(options, params, prefix='lstm'):
    ...
    if (options['lstm_arch_include_delta_t']):
...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    # mu = -log(1-baserate)
    baserate = 1e-6
    logts = numpy.random.randn((n_hid))+1.0
    params[_p(prefix,'logts')] = logts.astype(config.floatX)
    logmu = numpy.log(-numpy.log(1-baserate)) * numpy.ones((n_hid))
    params[_p(prefix, 'logmu')] = logmu.astype(config.floatX)
    logalpha = numpy.log(0.5) * numpy.ones((n_hid))
    params[_p(prefix, 'logalpha')] = logalpha.astype(config.floatX)
    logpriorexp = numpy.log(.0001) * numpy.ones((n_hid))
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    n_hid = options['n_hid']
    gamma = (1.0 / tensor.exp(tparams[_p(prefix, 'logts')])).reshape((1,-1,1))
    n_timescales = 1 # DEBUG HACK
	...
	t = timescale_prob.dimshuffle([1,0,2]).flatten(ndim=2).dimshuffle([1,0])
        return tensor.batched_dot(i, t).reshape([n_examples, n_hid])
...
def pred_error(f_pred, f_cost, prepare_data, data, iterator, verbose=False):
	...
	cost += f_cost(x, xt, y, yt, mask) 
        ...
        # DEBUG: for type-token prediction
        global tt_warn
        if (tt_warn>0):
           print('WARNING: type-token function')
           tt_warn -= 1
        correct_ct += (numpy.logical_and(y>0, (y-1)//5 == (preds-1)//5)).sum()
	all_ct += (y > 0).sum()
...
def train_model(
    ...
    hpm_arch_recurrent=False, # include recurrent connections between memory units
    lstm_arch_include_delta_t = True, #include delta_t values in lstm input 
    ...
    if model_options['encoder'] == 'lstm':
        ...
        model_options['arch_remap_input'] = True # force remapping
        model_options['arch_remap_output'] = True
    elif model_options['encoder'] == 'hpm':
        layers = {'hpm': (param_init_hpm, hpm_layer)}
    else:
        sys.exit("Invalid encoder");

    load_data = music.load_data;
    prepare_data = music.prepare_data;
    ...
    # for both n_in and n_out we count index 0 as a possible no-op input or
    # output
    model_options['n_out'] = numpy.max(list(chain(*train[Y]))) + 1 
    ...
    if (not model_options['arch_remap_input'] and
                                               model_options['hpm_arch_recurrent']):
...
if __name__ == '__main__':
    ...
    for i in range(1,n_datasets+1):
        ...
        tr[i-1], va[i-1], te[i-1] = train_model(encoder='lstm',
                   lstm_arch_include_delta_t=True, n_hid=25, data_file=filename)
        print(1-te)
