...
#theano.config.mode = 'FAST_COMPILE'        # DEBUG ************************
############################################################################
...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    baserate = 10.0**(-numpy.random.rand(n_hid)*7)
    mu = -numpy.log(1-baserate) # mu = -log(1-baserate)
    params[_p(prefix, 'logmu')] = numpy.log(mu).astype(config.floatX)

    # DEBUG
    #alpha = numpy.minimum(1.0, 
    #             numpy.random.exponential(scale=0.5,size=n_hid)) # scale = mean
    #params[_p(prefix, 'logalpha')] = numpy.log(alpha).astype(config.floatX)

    priorexp = 10.0**(-5.0*numpy.random.rand(n_hid))
    params[_p(prefix, 'logpriorexp')] = numpy.log(priorexp).astype(config.floatX)

    # initialize embedding for sparsity
    if (options['arch_remap_input']):
        randn = numpy.random.rand(options['n_in'],
                                  options['n_hid'])
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        if (options['arch_remap_input']):
            ...
            if (options['arch_hpm_recurrent']):
                ...
                preact += tensor.dot(marginal_intensity_input, tparams[_p(prefix,'U')])
                # m_i_i: n_examples X n_hid
                # U:     n_hid X 2n_hid 
            event = tensor.nnet.sigmoid(_slice(preact, 0, n_hid))
            gate  = tensor.nnet.sigmoid(_slice(preact, 1, n_hid))

            event = event * gate # apply the input gate
    ...
    rval, updates = theano.scan(_step,
							   ...
							   n_timescales,
                                ...
                                name=_p(prefix, '_layers'),
                                n_steps=nsteps)
    return rval[2], rval[0], rval[1] # DEBUG  # return yhat
...
def build_model(tparams, options):
    ...
    #DEBUG added h,c
    proj,h,c = get_layer(options['encoder'])[1](tparams, emb, xt, yt, options,
                                            prefix=options['encoder'],
                                            mask=mask)
    ...
    return use_noise, x, xt, y, yt, mask, pred_prob, corr, cost, proj, emb, h, c
    # DEBUG added proj, emb, h, c
...
def train_model(
    ...
    type_token_sim = False # sim that lumps together groups of 5 outputs DEBUG
):
    ...
    (use_noise, x, xt, y, yt, mask, 
           pred_prob, corr, cost, proj, emb, h, c) = build_model(tparams, model_options)
    # DEBUG ADDED PROJ, EMB, h, c
    # DEBUG NEW FUNCTION:
    ...
    try:
        for eidx in range(1,max_epochs+1):
            ...
            for _, train_index in kf:
                ...
                #h = f_h(x,xt,yt,mask)
                #c = f_c(x,xt,yt,mask)
                #for dd in range(0,24):
                #    print("hidden",dd,"maxh",numpy.max(h[:,:,:,dd]))
                ...
                #print("h[last,23]:\n",h[12,:,:,23])
                #print("c[23]:\n",c[:,:,:,23])
                #p = f_proj(x,xt,yt,mask)
                #print("proj shape ", p.shape, "Proj:\n",p)
                #print(numpy.isnan(p))
                #print_params = unzip(tparams)
                #print('Wemb[:,23]:\n',print_params['Wemb'][:,23])
