...
def param_init_hpm(options, params, prefix='hpm'):
    ...
    ts = 10.0**(numpy.random.rand(n_hid)*4.0-1.0)
...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    n_hid = options['n_hid']
    gamma = (1.0 / tensor.exp(tparams[_p(prefix, 'logts')])).dimshuffle(('x',0))
    mu = (tensor.exp(tparams[_p(prefix, 'logmu')])).dimshuffle(('x',0)) 
    # NOTE: alpha is scaled by gamma to force longer time scales to have
    # smaller alpha values
    alpha = gamma * (tensor.exp(tparams[_p(prefix,'logalpha')])).dimshuffle(('x',0))

    def _event_prob(intensity, delta_t):
        gamma_exp = numpy.exp(-gamma * delta_t)   # n_example x n_hid
        gamma_factor = (1.0 - gamma_exp) / gamma  # n_example x n_hid
	...
	# event dimensions: n_training_examples n_hid
	...
	# predict next event conditioned on timescale
        hhat, zhat = _event_prob(h, delta_t_output[:,None])
