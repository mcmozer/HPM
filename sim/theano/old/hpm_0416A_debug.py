...
def pred_error(f_corr, f_cost, prepare_data, data, iterator, options, f_pred_prob, f_proj, f_h, f_c, verbose=False):
    """
...
def train_model(
    ...
    f_pred_prob = theano.function([x, xt, yt, mask], 
                                ...
                                #pred_prob, name='f_pred_prob', mode='DebugMode')
    ...
    try:
        for eidx in range(1,max_epochs+1):
            ...
            for ix, train_index in kf:
                ...
                if (ix == 20):
                    ...
                    print('xt:\n',xt)
                    print('yt:\n',yt)
                    if (any(math.isnan(r) for r in g[3])):
                        print("NAN FOUND")
                    exit
		...
		train_err, train_cost = pred_error(f_corr,f_cost, 
                                            prepare_data,train,kf,model_options,f_pred_prob, f_proj, f_h, f_c)
		valid_err, valid_cost = pred_error(f_corr, f_cost, 
                                      prepare_data,valid,kf_valid,model_options,f_pred_prob, f_proj, f_h, f_c)
		test_err, test_cost = pred_error(f_corr,f_cost, 
                                        prepare_data,test,kf_test,model_options,f_pred_prob, f_proj, f_h, f_c)
    ...
    train_err, train_cost = pred_error(f_corr,f_cost,prepare_data,
                                            train,kf_train_sorted,model_options, f_pred_prob, f_proj, f_h, f_c)
    valid_err, valid_cost = pred_error(f_corr,f_cost,prepare_data, 
                                                   valid,kf_valid,model_options, f_pred_prob, f_proj, f_h, f_c)
    test_err, test_cost  = pred_error(f_corr,f_cost,prepare_data, 
                                                     test,kf_test,model_options, f_pred_prob, f_proj, f_h, f_c)
