...
def param_init_ctgru(options, params, prefix='ctgru'):
    ...
    Ud = ortho_weight(n_hid)
    Ur = ortho_weight(n_hid)

    # augment GRU input feature weights to include delta_t values
    if (options['arch_ctgru_include_delta_t']):
       Us = numpy.concatenate([Us, .01 * numpy.random.randn(2,n_hid)],axis=0)
       Ud = numpy.concatenate([Ud, .01 * numpy.random.randn(2,n_hid)],axis=0)
       Ur = numpy.concatenate([Ur, .01 * numpy.random.randn(2,n_hid)],axis=0)

    params[_p(prefix, 'Us')] = Us
    params[_p(prefix, 'Ud')] = Ud
    params[_p(prefix, 'Ur')] = Ur

    # remapping of the embedding 

    # set initial bias for the decay over the full range of time scales
    g0min = numpy.log(1.0/numpy.min(timescales))
    g0max = numpy.log(1.0/numpy.max(timescales))
    g0 = numpy.random.rand(n_hid,)*(g0max-g0min)+g0min 
    b = numpy.concatenate([numpy.zeros(n_hid,),             # s - event signal
                           g0 * numpy.ones(n_hid,),         # d - diffusion rate
                           numpy.zeros(n_hid,)], axis=0)    # r - recurrence gate
    params[_p(prefix, 'b')] = b.astype(config.floatX)
...
def ctgru_layer(tparams, state_below, xt, yt, options, prefix='ctgru', mask=None):
    ...
    def _step(m, x_, delta_t_input, delta_t_output, h_, y_):
        ...
        # include input and output delta_t values as GRU input features
        if (options['arch_ctgru_include_delta_t']):
            y_aug = tensor.concatenate([y_, delta_t_input[:,None], 
                                        delta_t_output[:,None]], axis=1)
        else:
            y_aug = y_

        # determine log(diffusion rate)
        d = _slice(x_, 1, n_hid) + tensor.dot(y_aug, tparams[_p(prefix,'Ud')])

        # determine recurrence gate
        r = tensor.nnet.sigmoid(_slice(x_, 2, n_hid) +
                     tensor.dot(y_aug, tparams[_p(prefix,'Ur')]))

        # apply recurrence gate to hidden
        ry_ = r * y_
        if (options['arch_ctgru_include_delta_t']):
            ry_aug = tensor.concatenate([ry_, delta_t_input[:,None], 
                                     delta_t_output[:,None]], axis=1)
        else:
            ry_aug = ry_

        # detect event signal
        s = tensor.tanh(_slice(x_, 0, n_hid) +
                     tensor.dot(ry_aug, tparams[_p(prefix,'Us')]))

        # credit assignment to timescales
        c = tensor.exp(-(d.dimshuffle((0,'x',1)) - loggamma) ** 2.0)
        c = c / tensor.sum(c,axis=1,keepdims=True)

        # update memory 

	hhat = (1.0 - c) * h_ + c * s.dimshuffle((0,'x',1))
...
def train_model(
    ...
    arch_ctgru_include_delta_t = False, #include delta_t values in gru input 
