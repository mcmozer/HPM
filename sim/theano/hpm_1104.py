...
def hpm_layer(tparams, state_below, xt, yt, options, prefix='hpm', mask=None):
    ...
    def _step(m, state_below, delta_t_input, delta_t_output, h_, c_, yhat_):
        ...
        if (not options['arch_remap_input']):
            ...
            if (options['arch_hpm_recurrent']):
                ...
                #off = 1e-30
                #if marginal_intensity_input.dtype == 'float16':
                #    off = 1e-5
                #marginal_intensity_input += off 
                # now normalize over n_hid for each training example
                marginal_intensity_input = (marginal_intensity_input / 
                        tensor.sum(marginal_intensity_input,axis=1,keepdims=True) )
                net += tensor.dot(
	...
	# predict next event conditioned on timescale
        ...
        # ensure no division by zero
        #off = 1e-30
        #if marginal_intensity_output.dtype == 'float16':
        #    off = 1e-5
        #marginal_intensity_output += off 
        # now normalize over n_hid for each training example
        marginal_intensity_output = (marginal_intensity_output / 
                    tensor.sum(marginal_intensity_output,axis=1,keepdims=True) )
