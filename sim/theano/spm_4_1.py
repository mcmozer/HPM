...
def init_params(options):
    ...
    if (options['arch_output_fn'] != '1-1'):
       params['U'] = (init_wt_mag / numpy.sqrt(options['n_out']) * 
                                  numpy.random.randn(2*options['n_hid'],
...
def param_init_lstm(options, params, prefix='lstm'):
    ...
    U = numpy.concatenate([numpy.zeros((2*n_hid, n_hid)),
                           numpy.zeros((2*n_hid, n_hid)),
                           numpy.zeros((2*n_hid, n_hid)),
                           numpy.zeros((2*n_hid, n_hid))], axis=1)
    ...
    if (options['arch_lstm_include_delta_t']):
       U = numpy.concatenate([U, 
                      .01 * numpy.random.randn(2,4*n_hid)],axis=0)
...
def lstm_layer(tparams, state_below, xt, yt, options, prefix='lstm', mask=None):
    ...
    def _step(m_, x_, delta_t_input, delta_t_output, h_, c_, cbar_):
        ...
        cbar = m_[:, None] * cbar + (1. - m_)[:, None] * cbar_

        if (options['arch_lstm_include_output_gate']):
            o = tensor.nnet.sigmoid(_slice(preact, 2, options['n_hid']))
            h = tensor.concatenate([o*tensor.tanh(c), o*tensor.tanh(cbar)],
                                   axis=1)
        else:
            h = tensor.tanh(tensor.concatenate([c,cbar],axis=1))
    ...
    rval, updates = theano.scan(_step,
                                ...
                                outputs_info=[tensor.alloc(numpy_floatX(0.), #h
                                              ...
                                              tensor.alloc(numpy_floatX(0.), #c
                                                           n_samples,
                                                           n_hid),
                                              tensor.alloc(numpy_floatX(0.), #cbar
                                                           n_samples,
                                                           n_hid)],
...
def param_init_spm(options, params, prefix='spm'):
    ...
    gamma0 = 1.0 / options['arch_spm_init_forget_timescale']
    if (options['arch_spm_rate_function'] == 'softplus'):
        if (gamma0 > 100.):
            forget0 = gamma0
        else:
            forget0 = numpy.log(numpy.exp(gamma0)-1.0) 
    elif (options['arch_spm_rate_function'] == 'exp'):
        forget0 = numpy.log(gamma0)
    else:
        print('Error: invalid arch_spm_rate_function')
        quit()

    # determine initial absorption rate (assuming only bias is nonzero)
    lambda0 = 1.0 /  options['arch_spm_init_input_timescale']
    if (options['arch_spm_rate_function'] == 'softplus'):
        input0 = numpy.log(numpy.exp(lambda0)-1.0)
    else:
        input0 = numpy.log(lambda0)

    # biases are ordered: 
    # (1) input gate, (2) decay const, (3) absorption, (4) input gate 
                          
    b = numpy.concatenate([input0 * numpy.ones((n_hid)),         # absorption rate
                           forget0 * numpy.ones((n_hid)),        # decay rate
                           0.0 * numpy.ones((n_hid)),            # input
                           0.0 * numpy.ones((n_hid))], axis=0)   # input gate
    params[_p(prefix, 'b')] = b.astype(config.floatX)
...
def spm_layer(tparams, state_below, xt, yt, options, prefix='spm', mask=None):
    ...
    def _step(m_, x_, delta_t_input, delta_t_output, o_, hbar_, h_, g_):
        # o_, h_, g_ have dim n_training_examples  X  n_spm
        # delta_t_input has dim n_training_examples
        ...
        # x_ = W * x_t + b
        preact = x_ + tensor.dot(o_, tparams[_p(prefix, 'U')]) 

        # determine time constants moving forward
        if (options['arch_spm_rate_function'] == 'softplus'):
            lam = tensor.nnet.softplus(_slice(preact,0,n_hid))
            gamma = tensor.nnet.softplus(_slice(preact,1,n_hid))
        else:
            lam = tensor.exp(_slice(preact,0,n_hid))
            gamma = tensor.exp(_slice(preact,1,n_hid))

        # compute event signal
        z = tensor.tanh(_slice(preact,2,n_hid))

        if (options['arch_spm_include_input_gate']):
            gate = tensor.tanh(_slice(preact,3,n_hid))
            z *= gate
        ...
        lam_decay = tensor.exp(-lam * delta_t_output[:, None])
        gamma_decay = tensor.exp(-gamma * delta_t_output[:, None])
        h = gamma_decay * h_ + lam/(gamma-lam)*(lam_decay-gamma_decay) * gprime
        g = lam_decay * gprime
        # hbar represents forgotten activation (difference between h with
        # gamma=0 and given gamma)
        hbar = hbar_ + (gprime - g) + (h_ - h)

        # generate output from SPM layer
        o = tensor.tanh(tensor.concatenate([h,hbar],axis=1))

        o = m_[:, None] * o + (1. - m_)[:, None] * o_
        hbar = m_[:, None] * hbar + (1. - m_)[:, None] * hbar_
        h = m_[:, None] * h + (1. - m_)[:, None] * h_
        g = m_[:, None] * g + (1. - m_)[:, None] * g_
    ...
    rval, updates = theano.scan(_step,
                        sequences=[mask, state_below, xt, yt],
                        outputs_info=[tensor.alloc(numpy_floatX(0.), # o_
                                                       n_samples,
                                                       n_hid*2),
                                          tensor.alloc(numpy_floatX(0.), # hbar_
                                                       n_samples,
                                                       n_hid),
                                          tensor.alloc(numpy_floatX(0.), # h_
                                                       n_samples,
                                                       n_hid),
                                          tensor.alloc(numpy_floatX(0.), # g_
                                                       n_samples,
                                                       n_hid)],
                        name=_p(prefix, '_layers'),
                        n_steps=nsteps)
...
def train_model(
    ...
    arch_spm_rate_function = 'softplus', # softplus or exp 
    arch_spm_include_input_gate = True, # input gate
    arch_spm_init_input_timescale = 1.0, # intial absorption rate
    arch_spm_init_forget_timescale = 100.0, # initial forgetting rate
    ...
    elif model_options['encoder'] == 'lstm':
